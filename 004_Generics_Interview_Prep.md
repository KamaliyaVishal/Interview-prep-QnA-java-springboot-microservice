...
3. Reusable algorithms and data structures
4. Stronger API contracts
5. Fewer runtime `ClassCastException` problems

### Senior answer

> Generics move many type errors from runtime to compile time and allow reusable, type-safe APIs without scattering casts throughout the code.
...
`String` is supplied as the actual type.
...
Bounds restrict which types may be used as a generic argument.
...
because two class bounds are not allowed.
...
They allow generic code to use operations guaranteed by the bound.
...
Because `T` is known to be a `Number`, the compiler allows:

```java
value.doubleValue();
```

Without the bound, that operation could not be assumed.
...
Means the list is specifically parameterized with `Object`.
...
Means the list contains some unknown type.
...
is not safe because the actual list could be `List<Integer>`.
...
Use it when the method works with a collection of **any type** but does not need to add values of that unknown type.
...
> Some unknown type that is `T` or a subtype of `T`.
...
we could put an `Integer` into a `List<Double>`.

Therefore arbitrary non-null additions are rejected.
...
because every possible element type extends `Number`.
...
You cannot add a specific non-null value:
...
because `null` is valid for every reference type.
...
> Some unknown type that is `T` or a supertype of `T`.
...
Every possible actual list can safely contain an `Integer`:
...
and the stored value could be a `String`.
...
If a structure **produces values** for your code to read:
...
If a structure **consumes values** that your code wants to put into it:
...
PECS helps design flexible generic APIs.

A copy operation can be expressed as:
...
This is the same fundamental design used by APIs such as `Collections.copy()`.
...
Every element is therefore at least a `Number`:

```java
Number n = list.get(0);
```

But the compiler cannot know which exact subtype the list requires when adding.
...
Java implements generics primarily through **compile-time type checking and type erasure**.
...
use the same runtime `ArrayList` class.

The compiler uses generic information for type checking and inserts casts where required.
...
involves retrieving an object and enforcing the expected `String` type through the generated bytecode.
...
A major reason is **backward compatibility**.

Generics were introduced in Java 5 while preserving compatibility with existing Java bytecode and pre-generics APIs.

This allowed existing code using:

```java
List
```

to continue working with newer code using:

```java
List<String>
```

with compiler warnings where type safety could not be guaranteed.

### Senior answer

> Erasure allowed Java to add generic type checking without requiring a completely separate runtime representation for every parameterized type, helping preserve compatibility with the existing JVM ecosystem.
...
Not in the same way they are available to the compiler.
...
Because the runtime does not know which concrete type `T` represents after erasure.
...
```java
Factory<StringBuilder> factory =
        new Factory<>(StringBuilder::new);
```
...
Arrays are **reified**: the runtime knows their component type.

Generic type parameters are subject to erasure.
...
The runtime cannot determine whether the array should be:
...
A collection is usually preferable:
...
The runtime knows that the array is a `String[]`.

This enables runtime array-store checks:
...
Generics are mostly erased:

```java
List<String>
```

does not have a separate runtime class from:

```java
List<Integer>
```

This difference explains restrictions around generic arrays.
...
Because covariance for mutable collections would break type safety.
...
would put a `Double` into a list intended to contain `Integer`.
...
A raw type uses a generic class without specifying its type argument:
...
Heap pollution occurs when a variable of a parameterized type refers to an object that is not actually of the expected parameterized type.
...
The raw reference bypasses generic type checking.
...
can fail depending on the element retrieved.

Heap pollution can arise from:
...
Arrays are reified, while generic type arguments are erased.
...
can cause an unchecked warning because the runtime cannot fully represent the parameterized array type.

`@SafeVarargs` can suppress the warning when the method is genuinely safe:
...
> `@SafeVarargs` does not make unsafe code safe. It tells the compiler that the implementation does not perform unsafe operations involving the varargs parameter.
...
The compiler can infer generic type arguments from context.
...
A recursive bound references the type parameter itself.
...
> `T` must be comparable with another `T`.
...
Because we want a type-safe relationship:
...
instead of relying on the raw `Comparable` type.
...
> Give me a list of some unknown type. I don't need to name that type.
...
> There is a specific type `T`, and these parameters are related through that same type.

Use a type parameter when the type relationship needs to be expressed across parameters or a return value.
...
Use a wildcard when the type is unknown and no relationship involving that type needs to be expressed.

```java
void print(List<?> list)
```

Use a type parameter when you need a relationship:
...
> Use a wildcard when the type is unknown and irrelevant to the relationship; use a type parameter when the type itself participates in a relationship.
...
Its generic contract follows the PECS model:
...
Therefore the source may contain a subtype of `T`, while the destination can consume `T` or a supertype.

This is an excellent real-world example of PECS.
...
Generics are common in enterprise abstractions.
...
If I need to add values, I would identify the actual type relationship and use a suitable type parameter or lower-bounded wildcard.

The key is:

> Design the generic contract around the operation rather than adding wildcards blindly.
...
`List<Object>` is not a supertype of every parameterized `List`.
...
If the method only needs to inspect values:
...
If it consumes a specific type:
...
I would migrate toward parameterized types, prioritizing public APIs and high-risk code paths first.
...
Because after erasure both have approximately:

```java
void process(List list)
```

Generic type arguments cannot distinguish these overloads.
...
For example, a class cannot implement both:
...
because that would create conflicting parameterizations of the same generic interface.
...
PECS is a guideline for designing flexible APIs.

Adding wildcards everywhere can make APIs harder to understand.
...
may be clearer when the exact type relationship matters.

The goal is:

> Maximize useful flexibility without making the API unnecessarily complex.
...
may be technically valid but difficult to understand and use.

Introduce bounds when they provide meaningful operations or enforce an important contract.
...
**Safe to read as `Number`; cannot add arbitrary non-null values.**
...
**Can add `Integer`; safe reads are `Object`.**
